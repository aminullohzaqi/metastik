    ...
    <div class="main">
        <?php
             if (isset($_GET['scheck'])){
                ...
                AND '".$_GET['endDate']."' ORDER BY  `Date` ASC");
        ...
        ?> 
        <h3 class="text-center">Step Check Stasiun <?php echo mysqli_fetch_array($sql)['Stasiun'];?></h3>
        <br>
        <table id="tabel">
            <tr>
            ...
            <?php 
            $previousValueTmed        = null;
            $previousValueTmax        = null;
            $previousValueTmin        = null;
            $previousValuePrec        = null;
            $previousValuePress       = null;
            $previousValueInsolation  = null;
            while ($row=mysqli_fetch_array($sql)){ ?>
            <tr>
                <td><?php echo $row['Date'];?></td>
                <td><?php echo $row['Tmed'];?></td>
                <td><?php
                    if ($previousValueTmed){
                        $Quality = abs($previousValueTmed - $row['Tmed']);
                        if ($Quality > 5){
                            echo "Doubtful";
                        } else{
                            echo "Normal";
                    ...
                    $previousValueTmed = $row['Tmed'];
                    ?></td>
                <td><?php echo $row['Tmax'];?></td>
                <td><?php
                    if ($previousValueTmax){
                        $Quality = abs($previousValueTmax - $row['Tmax']);
                        if ($Quality > 5){
                            echo "Doubtful";
                        } else{
                            echo "Normal";
                    ...
                    $previousValueTmax = $row['Tmax'];
                    ?></td>
                <td><?php echo $row['Tmin'];?></td>
                <td><?php
                    if ($previousValueTmin){
                        $Quality = abs($previousValueTmin - $row['Tmin']);
                        if ($Quality > 5){
                            echo "Doubtful";
                        } else{
                            echo "Normal";
                    ...
                    $previousValueTmin = $row['Tmin'];
                    ?></td>
                <td><?php echo $row['Prec'];?></td>
                <td><?php
                    if ($previousValuePrec){
                        $Quality = abs($previousValuePrec - $row['Prec']);
                        if ($Quality > 50){
                            echo "Doubtful";
                        } else{
                            echo "Normal";
                    ...
                    $previousValuePrec = $row['Prec'];
                    ?></td>
                <td><?php echo $row['Press'];?></td>
                <td><?php
                    if ($previousValuePress){
                        $Quality = abs($previousValuePress - $row['Press']);
                        if ($Quality > 2){
                            echo "Doubtful";
                        } else{
                            echo "Normal";
                    ...
                    $previousValuePress = $row['Press'];
                    ?></td>
                <td><?php echo $row['Insolation'];?></td>    
                <td><?php
                    if ($previousValueInsolation){
                        $Quality = abs($previousValueInsolation - $row['Insolation']);
                        if ($Quality > 5){
                            echo "Doubtful";
                        } else{
                            echo "Normal";
                    ...
                    $previousValueInsolation = $row['Insolation'];
                    ?></td>    
            </tr>
            <?php } ?>
        </table>
